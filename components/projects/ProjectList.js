...
const mutationCreateProject = gql`
	mutation CreateProject(
		...
		createProject(owner: $owner, title: $title, description: $description, status: $status, tasks: $tasks) {
			id
		}
	}
`;
...
class ProjectList extends Component {
	...
	handleClose = () => {
		this.setState({ open: false, title: 'Default Title', description: 'Default Description' });
	};
	...
	render() {
		...
		return (
			<div className="card-display">
				<div>
					<RaisedButton label="CREATE NEW PROJECT" onClick={this.handleOpen} style={style} primary={true} />
					...
					>
						...
						>
							{users.map((user) => <MenuItem key={user.id} value={user.id} primaryText={user.name} />)}
						</SelectField>
					</Dialog>
				</div>
							...
							<Mutation mutation={mutationDeleteProject}>
								{(deleteProject) => (
									<FlatButton
										...
										label="DELETE"
										secondary={true}
										onClick={(event) => this.handleDeleteProject(event, deleteProject)}
									/>
								)}
							</Mutation>
